...
class ThunkUploader(val callingActivity: Activity, val thunkView: TextView) : AsyncTask<Void, Void, AsyncTaskResult<Thunk>>() {
    override fun doInBackground(vararg params: Void): AsyncTaskResult<Thunk> {
        val getResult: AsyncTaskResult<Thunk> = AsyncTaskResult()
//        try {
//            val parsedThunk = Thunk(URL("http://10.0.2.2:5000/thunk").(Charset.defaultCharset()))
